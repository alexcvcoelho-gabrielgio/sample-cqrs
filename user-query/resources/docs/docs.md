...
The `home-routes` handler in the `user-query.routes.home` namespace
...
The `home-page` function will in turn call the `user-query.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `user-query.handler` namespace:
...
Request middleware functions are located under the `user-query.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `user-query.dev-middleware` namespace found in
...
* Let `mount` know to start the database connection by `require`-ing `user-query.core` in some other namespace.
